...
//Buscar o caminho do pic na instalação
var tagLoadControl,
	lastScriptSrc,
	scriptsLocais,
	globalFolderPosition,
	pathPic;

tagLoadControl = document.querySelector('script[src$="load-control.js"]');
										
lastScriptSrc = (tagLoadControl === null) ? "" : tagLoadControl.getAttribute("src"); 

scriptsLocais = ( tagLoadControl.getAttribute('data-pic-customscripts') || "");

globalFolderPosition = lastScriptSrc.indexOf("pic"); 

pathPic = lastScriptSrc.substr(0, globalFolderPosition)+'pic/';

PIC.path = pathPic;

// Parametros de configurações YUI //
YUI_config = {
	base: pathPic+'js/',
};
...
YUI().use('get', function (Y) {
	...
	{onSuccess: function () {
		...
		Y.Get.css([pathPic+'css/tema_basico.css', pathPic+'css/tema_comum.css'], {
			onFailure: function () {
				Y.log('CSS dos plugins: Carregamento cancelado...');
			},  
			onSuccess: function () {
		...
		Y.Get.js(pathPic+'js/pic.allcustoms.js', {
			...
			onSuccess: function () {
				...
				if (scriptsLocais.length) {
					scriptsLocais = scriptsLocais.replace("[", "").replace("]", "").split(",");
					
					//sequencia a carga e execução da lista de scripts locais
					$.each(scriptsLocais, function(i) {
						$(document).ready( function() {
							Y.Get.js(scriptsLocais[i], {
								onFailure: function () {
