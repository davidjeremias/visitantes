  ...
  <span>
          ...
          <div class="row main-info-container">
            <div class="col-md-2 text-center photo-avatar-container" data-toggle="modal" data-target="#photoWebcam">
              <div class="photo-avatar" id="photo-avatar" v-on:click="startup()">
                <img v-bind:src="data" class="photo-webcam" alt="Perfil" />
              </div>
            </div>
              ...
              <div class="row">
                ...
                <div class="col-md-8">
                  <div class="form-group">
                    ...
                    <input type="text" v-on:keyup="validaRegistraEntradaButton()" v-model="nomeVisitante" class="form-control" id="sivis-nome" />
                  </div>
                </div>
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    <label for="sivis-entrada">Entrada</label>
                    <input type="text" v-model="dataHoraVisita" class="form-control" id="sivis-entrada" disabled />
                  </div>
                </div>
    ...
    <div class="row margin-top-row">
        <div class="col-md-12">
            <button
                type="button"
                v-if="$auth('ATUALIZAR_INFORMACOES_DO_REGISTRO_DE_ENTRADA_PELO_CADASTRADOR') || $auth('ATUALIZAR_INFORMACOES_DO_VISITANTE_PELO_CADASTRADOR')"
                :disabled="!isRegistraEntrada"
                @click="confirmaAlterar()"
                class="btn btn-primary button-busca-margin pull-right">
                Salvar
            </button>
    ...
    <div data-pic-modal='{"title": "Digitalizar Documento", "dialog": "default"}' id="novoDocumento">
      <form>
                ...
                <div class="row">
                  ...
                  <template v-if="!isEdit">
                    <div class="col-md-12 text-center">
                      <button
                        type="button"
                        class="btn btn-primary button-margin-top"
                        v-on:click="
                      ...
                      <button type="button" class="btn btn-secondary button-margin-top button-margin-left" v-on:click="startupDocumentoFrente('novo', null, null)">
                  ...
                  <template v-if="isEdit">
                    <div class="col-md-12 text-center">
                      <button
                        ...
                        v-on:click="
                          takepictureDocumentoFrente();
                          startupDocumentoVersoEditar('frente');
                        "
                      >
                        <i class="fas fa-camera"></i>
                      </button>
                      <button type="button" class="btn btn-secondary button-margin-top button-margin-left" v-on:click="startupDocumentoFrenteEditar(null, null)">
                        <i class="fas fa-redo"></i>
                      </button>
                    </div>
                  </template>
              ...
              <div class="col-md-6 document-webcam-container" id="documentoVersoContainer">
                <div class="row">
                  ...
                  <template v-if="isEdit">
                    <div class="col-md-12 text-center">
                      <button type="button" class="btn btn-primary button-margin-top" v-on:click="takepictureDocumentoVerso()"><i class="fas fa-camera"></i></button>
                      <button type="button" class="btn btn-secondary button-margin-top button-margin-left" v-on:click="startupDocumentoVersoEditar('verso')">
                        <i class="fas fa-redo"></i>
                      </button>
                    </div>
                  </template>
            ...
            <div class="row document-fields-container">
              ...
              <div class="col-md-6" v-if="isEdit">
                <div class="form-group">
                  <label for="origem">Origem</label>
                  <select
                    v-model="documentoEdicao.origemDocumento"
                    @change="
                      onChangePaises();
                      catchEvent($event);
                      changeDocument();
              ...
              <template>
                <div class="col-md-6">
                  ...
                  <div class="form-group" v-if="isEdit">
                    <label for="tipo-do-documento">Tipo do Documento</label>
                    <select
                      v-model="documentoEdicao.tipoDocumento"
                      class="form-control"
                      id="tipo-do-documento"
                      name="tipo-do-documento"
                      @change="
        ...
        <div class="buttons">
          	<button v-if="!isEdit" type="submit" id="digitalizarDocumentoConfirm" class="btn btn-primary" :disabled="isConfirmaDocumento || isDocumentoDuplicidade" @click="addDocumento()">
            	Adicionar
          	</button>
			<button v-if="isEdit" type="submit" id="digitalizarDocumentoConfirm" class="btn btn-primary" :disabled="isConfirmaDocumento || isDocumentoDuplicidade" @click="addDocumento()">
				Atualizar
	...
	props: {
  	...
  	data() {
		return {
			...
			listaNomesGabinetes: [],
			isEntradaLocal: null,
			isEntradaGabinete: null,
			gabineteDestinoSelecionado: null,
  	...
  	created: function() {
		...
		if (this.entrada) {
			this.id = this.entrada.visitanteDTO.id;
			this.data = this.entrada.visitanteDTO.fotoVisitante;
			this.nomeVisitante = this.entrada.visitanteDTO.nomeVisitante;
			this.numeroCPF = this.entrada.visitanteDTO.cpf;
			this.numeroTel = this.entrada.visitanteDTO.telefoneVisitante;
			this.dataAlteracaoFoto = this.entrada.visitanteDTO.dataAlteracaoFoto;
			this.dataHoraVisita = this.getTime(this.entrada.dataHoraVisita);

			this.listaDocumentosItems = this.entrada.visitanteDTO.documentos;

			this.dataHoraEntrada = this.entrada.dataHoraVisita;
			this.dataCadastroVisitante = this.entrada.visitanteDTO.dataCadastro;
			this.selectedPortaria = this.entrada.portaria;
			this.selectedDestino = this.entrada.destino;
			this.idEntrada = this.entrada.id;
			this.inexistenciaDocumento = this.entrada.inexistenciaDocumento ? this.entrada.inexistenciaDocumento : false;

			if (this.numeroCPF) {
				this.isControleCpf = true;
			}

			if (!this.entrada.destino) {
				this.isEntradaLocal = false;
				this.isEntradaGabinete = true;
			}

			this.$http.get(`/documento?parametros=${this.id}`).then(
				function(response) {
		...
		this.$http.get(`/portaria`).then(
	  	...
	  	if (!this.entrada.idGabinete) {
			this.isEntradaLocal = true;
			this.isEntradaGabinete = false;

			setTimeout( ()=>{
				document.getElementById("gabinete-destino").disabled = true;
			});
			...
			this.descGabineteSelecionado = this.entrada.gabineteDTO.label;
			this.gabineteDestinoSelecionado = this.entrada.idGabinete;
		}
  	},
  	methods: {
		verificarTelefone() {
		...
		validaApresentacaoInputDocumento: function(document) {
			return (
				document.tipoDocumento.descTipoDocumento !== "cnh" ||
				document.tipoDocumento.descTipoDocumento !== "oab" ||
				document.tipoDocumento.descTipoDocumento !== "dni" ||
				document.tipoDocumento.descTipoDocumento !== "conselho de classe"
			);
		},
		...
		selecionarGabineteDestino() {
			...
			let idGabineteSelect;
			let gabineteDestino = document.getElementById("gabinete-destino").value;
			this.descGabineteSelecionado = gabineteDestino;

			this.listaGabinetes.forEach(function(e, i) {
				if (e.label.trim() == gabineteDestino.trim()) {
					idGabineteSelect = e.value;
				}
			});

			if (gabineteDestino != "") {
				document.getElementById("local").disabled = true;
				this.gabineteDestinoSelecionado = idGabineteSelect;
				...
				document.getElementById("local").disabled = false;
				this.listaDestino.unshift(this.selecionarDestinos);
				this.gabineteDestinoSelecionado = null;
				this.descGabineteSelecionado = "";
				this.isRegistraEntrada = false;
			}
		...
		confirmaAlterar() {
			...
			this.$http.put(`/visitante`, visitanteDTO).then(
				function(response) {
					...
					if (response.status === 201) {
						this.$router.push({
							...
							params: {
								msg:`Dados do visitante ${this.nomeVisitante} atualizados com sucesso às ${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}:${String(now.getSeconds()).padStart(2, "0")}`,
								isSalvo: true
							}
						});
					}
		...
		validarCPF() {
			...
			if (rev != parseInt(cpf.charAt(9))) return (this.isValid = false);
			...
			rev = 11 - (add % 11);
			if (rev == 10 || rev == 11) rev = 0;
			if (rev != parseInt(cpf.charAt(10))) return (this.isValid = false);
		...
		catchEventCountry(event) {
			...
			if (
				this.tipoPaisSelecionado === "ARGENTINA (Mercosul)" ||
				this.tipoPaisSelecionado === "PARAGUAI (Mercosul)" ||
				this.tipoPaisSelecionado === "URUGUAI (Mercosul)" ||
				this.tipoPaisSelecionado === "VENEZUELA (Mercosul)"
			) {
		...
		startupDocumentoFrenteEditar(elemento, index) {
			...
			this.videoDocumentoFrente.addEventListener(
				"canplay",
				() => {
				if (!this.streamingDocumentoFrente) {
					this.heightDocumentoFrente = this.videoDocumentoFrente.videoHeight / (this.videoDocumentoFrente.videoWidth / this.widthDocumentoFrente);
		...
		startupDocumentoVerso() {
			...
			this.videoDocumentoVerso.addEventListener(
				"canplay",
				() => {
		...
		validaAutorizarButton() {
			if (this.data == null || this.nomeVisitante == null || this.selectedPortaria.id == null || (this.selectedDestino == null && this.gabineteDestinoSelecionado == "")) {
				return true;
			} else {
				return false;
			}
		...
		validaCPF() {
			...
			if (this.numeroCPF != "" || this.numeroCPF != null) {
				...
				.then(
					...
					if (response.data) {
						if (response.data.nomeVisitante === this.nomeVisitante) {
						this.isDuplicidade = false;
						}
					}
					this.validaRegistraEntradaButton();
		...
		validaCarteiraDeTrabalho() {
			if (this.numeroCPF != "") {
				...
				this.$http.get(`/documento/validaCPF?parametros=${this.numeroCPF}`).then(
					function(response) {
						if (response.status === 200) {
							this.isDuplicidade = true;
						} else {
			...
			if (this.numero && this.dataDocumentoFrente == "" && this.selectedEstado != "" && this.numeroCPF) {
				this.isConfirmaDocumento = false;
			} else {
				this.isConfirmaDocumento = true;
			}
